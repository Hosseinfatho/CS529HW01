    ...
    const mapGroupSelection = useMemo(()=>{
        ...
        if(svg !== undefined && props.map !== undefined && props.data !== undefined){
            ...
            function drawLegend(){
                ...
                // Scale down the legend to 0.9 to fit better in SVG
                const barHeight = Math.min(height/10,40) * 0.9;
                
                let legendX = bounds.x + 10 + bounds.width;
                const barWidth = Math.min((width - legendX)/3,40) * 0.9;
                const fontHeight = Math.min(barWidth/2,16) * 0.9;
